...
public class Banco {
    private static File arquivo = new File("C:\\Users\\Guilherme\\Desktop\\Cimol 2024\\Programação\\aula23");
    public static void gravarContas(Conta[] lista) {

        try {
            ObjectOutputStream arquivoEscrita =
                    new ObjectOutputStream(new FileOutputStream(arquivo.getAbsolutePath()));
            arquivoEscrita.defaultWriteObject(lista);
            arquivoEscrita.close();

        } catch (IOException e) {
            System.out.println("Erro ao gravar arquivo");
        }
        public static Conta[] lerContas() {
            ObjectInputStream arquivoLeitura = new ObjectInputStream(new FileInputStream(arquivo.getAbsolutePath()));
        }
    }
